...
export default function AddCustomPlayerCard() {
  ...
  return (
    <div className="card flex flex-col items-center gap-3 border-2 border-amber-300 bg-gradient-to-br from-amber-50/80 to-orange-50/50 p-6 text-center">
      <div className="text-xs font-black uppercase tracking-widest text-amber-600">Voeg eerst je profielspeler toe</div>
      ...
      <div>
        ...
        <div className="text-xs text-amber-700">{POS_LABEL[player.position]} · Level {player.level}</div>
      </div>
      <button
      ...
      >
        Voeg {player.name} toe aan je opstelling
      </button>
    </div>
  );
}
